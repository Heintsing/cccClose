...
def Image2hexTCPMod2(M):  # 适配超材料的标准为3*3 24*32
    ...
    M = np.flip(M, axis=1)
    # M = np.flip(M, 0)
    MM = np.concatenate((M, np.ones([8, 24])))
    # M = np.concatenate((M, np.ones([8, 24])))
    MM = MM.T
    MM[0:8, 0:8] = np.flip(MM[0:8, 0:8], 1)
    MM[0:8, 8:16] = np.flip(MM[0:8, 8:16], 1)
    MM[0:8, 16:24] = np.flip(MM[0:8, 16:24], 1)
    MM[0:8, 24:32] = np.flip(MM[0:8, 24:32], 1)
    MM[8:16, 0:8] = np.flip(MM[8:16, 0:8], 1)
    MM[8:16, 8:16] = np.flip(MM[8:16, 8:16], 1)
    MM[8:16, 16:24] = np.flip(MM[8:16, 16:24], 1)
    MM[8:16, 24:32] = np.flip(MM[8:16, 24:32], 1)
    MM[16:24, 0:8] = np.flip(MM[16:24, 0:8], 1)
    MM[16:24, 8:16] = np.flip(MM[16:24, 8:16], 1)
    MM[16:24, 16:24] = np.flip(MM[16:24, 16:24], 1)
    MM[16:24, 24:32] = np.flip(MM[16:24, 24:32], 1)

    MM[0:8, 0:8] = np.flip(MM[0:8, 0:8], 0)
    MM[0:8, 8:16] = np.flip(MM[0:8, 8:16], 0)
    MM[0:8, 16:24] = np.flip(MM[0:8, 16:24], 0)
    MM[0:8, 24:32] = np.flip(MM[0:8, 24:32], 0)
    MM[8:16, 0:8] = np.flip(MM[8:16, 0:8], 0)
    MM[8:16, 8:16] = np.flip(MM[8:16, 8:16], 0)
    MM[8:16, 16:24] = np.flip(MM[8:16, 16:24], 0)
    MM[8:16, 24:32] = np.flip(MM[8:16, 24:32], 0)
    MM[16:24, 0:8] = np.flip(MM[16:24, 0:8], 0)
    MM[16:24, 8:16] = np.flip(MM[16:24, 8:16], 0)
    MM[16:24, 16:24] = np.flip(MM[16:24, 16:24], 0)
    MM[16:24, 24:32] = np.flip(MM[16:24, 24:32], 0)
